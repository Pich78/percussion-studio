...
- **Before running the E2E suite, always verify port 8000 is free**: `ss -tlnp | grep 8000` (or `lsof -i :8000`). If something is listening (e.g., a stale `launch_local.py` or `test_launch_local.py` from an earlier run), kill it (`pkill -f launch_local.py`) **before** starting the suite — a stale server holding the port makes all 5 tests fail with the page stuck on the loading screen. After the suite finishes, kill any leftover server process the agent started (the runner stops its own server, but any manually launched `launch_local.py` must be terminated with `pkill -f launch_local.py`).
