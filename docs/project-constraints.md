...
|---------|---------|----------|---------|
| js-yaml | 4.1.0 | `js/vendor/js-yaml-4.1.0.min.js` | YAML parsing |
| Tailwind CSS | 3.4.17 (play build) | `js/vendor/tailwind-play-3.4.17.js` | CSS utility framework |

These are loaded as regular `<script>` tags in `mobile.html` and `desktop.html`, **not** as ES module imports. They are vendored (no runtime CDN) so the service-worker snapshot can hash-verify and cache the entire boot. See `js/vendor/README.md` for origins, licences and the update procedure.
...
| **Manifest** | `manifest.json` at project root is the central resource registry, auto-generated by `tools/generate_manifest.py`, which also emits the offline snapshot descriptor `precache.json` (version + per-asset hashes). | `docs/data-specifications.md`, `docs/requirements/offline-updates.md` |
...
| **Orientation changes** | Handle `orientationchange` + `visualViewport.resize` in addition to `resize` (`js/ui/mobileViewport.js`): orientation flips reset the document scroll and close orientation-scoped popovers (state cleanup + targeted removal of `[data-role="orientation-popover"]` subtrees). Rotation performs **no JS re-render**: the grid cell size is a pure CSS clamp (`--cell-size`) and safe areas are `env()`-backed, so the browser reflows by itself. The mobile shell is pinned (`html`/`body` overflow hidden, `#root` fixed) so rotation cannot move the headers out of view. | `js/ui/mobileViewport.js`, `js/utils/gridUtils.js`, `mobile.html`, `desktop.html` |
| **Offline & updates** | `sw.js` serves app assets from one atomic, hash-verified snapshot per generated version (`precache.json`): no mixed versions, full offline functionality (shell, YAML, WAVs). New versions are background-built and swapped only after verification; the page reloads when idle (never during playback). Registration is skipped on localhost unless `?sw=1`. | `docs/requirements/offline-updates.md`, `sw.js`, `js/app.js` |
...
| **Manifest regeneration** | Run `python3 tools/generate_manifest.py` after any data or app-asset changes; commit `manifest.json`, `data/rhythms/Batà/bata_metadata.json` and `precache.json` together. | `README.md`, `docs/adding-instruments.md`, `docs/requirements/offline-updates.md` |
