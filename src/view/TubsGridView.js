...
// Define layout constants in one place for easy maintenance.
static HEADER_WIDTH_PX = 80;
    ...
    // Use the static constant for the grid layout
    const headerWidth = TubsGridView.HEADER_WIDTH_PX;
    const gridStyles = `grid-template-columns: ${headerWidth}px repeat(${resolution}, 1fr);`;
    ...
    // Use the static constant for calculations
    const headerWidth = TubsGridView.HEADER_WIDTH_PX;
    const leftStyle = `calc(${headerWidth}px + (100% - ${headerWidth}px) * ${multiplier})`;
    const widthStyle = `calc((100% - ${headerWidth}px) / ${resolution})`;
