...
- **Live URL**: https://pich78.github.io/percussion-studio/
- **Platform**: GitHub Pages
- **PWA**: Can be added to iPhone home screen via Chrome → Share → Add to Home Screen
- **Testing**: Test on actual iPhone in PWA mode (standalone) for accurate safe area behavior
...
### Safe Area Handling (iPhone PWA)
For elements that need to avoid the iPhone notch and home indicator:

- Use `env(safe-area-inset-top)`, `env(safe-area-inset-bottom)`, `env(safe-area-inset-left)`, `env(safe-area-inset-right)` CSS functions
- Example: `pb-[calc(env(safe-area-inset-bottom,0px)+1rem)]`
- Test in PWA mode on actual iPhone for accurate behavior

### Browser Gesture Prevention (Critical for PWA)
This app runs as a PWA and must avoid native browser gestures for a native-like experience.

**Always add `touch-action: none` to interactive elements:**

```html
<!-- Range sliders -->
<input type="range" style="touch-action: none" ... />
...
**Why this matters:**
- iOS Safari interprets touches for scroll, zoom, pull-to-refresh
- Without `touch-action: none`, touches may trigger unwanted browser gestures
- Test on actual iPhone in PWA mode - gestures behave differently than in Safari
