...
export async function run() {
    ...
    runner.describe('TubsGridRenderer Module', () => {
        ...
        runner.it('createMeasureHeader() should create a header with correct content', () => {
            const el = TubsGridRenderer.createMeasureHeader({ metric: '12/8', resolution: 8 });
            runner.expect(el.classList.contains('measure-header')).toBe(true);
            runner.expect(el.querySelector('.metric').textContent).toBe('12/8');
            runner.expect(el.querySelector('.resolution').textContent).toBe('8th');
        });
