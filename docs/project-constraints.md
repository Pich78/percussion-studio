...
| Constraint | Details | Source |
|-----------|---------|--------|
| **No server-side logic** | The application is entirely static files. All data is loaded via `fetch()` using a `manifest.json` registry. | `docs/data-specifications.md:163` |
| **Data format** | All content data is in YAML files under `data/`. | `README.md` |
| **Manifest** | `manifest.json` at project root is the central resource registry, auto-generated by `tools/generate_manifest.py`. | `docs/data-specifications.md:163-172` |
