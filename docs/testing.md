...
| `mobile-landscape` | iPhone 16, Safari-like **734×343** | `e2e/mobile-landscape.spec.js` — landscape read-only grid, toggles play; a leftward swipe starting on the header still advances to the next section; symbol icons keep the CSS cell-size thresholds (24px on a ~33.7px cell). Plus shared `e2e/mobile-wheel-picker.spec.js` (same picker coverage). |
...
`e2e/mobile-rotation.spec.js` (shared by both PWA projects, driven by `helpers/rotation.js`) pins the app-side rotation invariants implemented in `js/ui/mobileViewport.js`: the visible header survives portrait → landscape → portrait, `#root` keeps filling the viewport, the document is not scrolled, orientation-scoped popovers close and never reappear on the way back, and rotation performs **zero full `#root` renders** (grid sizing is CSS-driven — cell width matches the `--cell-size` formula without a rebuild). It also covers an open rhythm browser across a rotation.
