      ...
      <header class="h-14 px-2 border-b border-gray-800 flex justify-between items-center bg-gray-950 flex-shrink-0 z-40 gap-2">
        ...
        <div class="flex items-center justify-center flex-1 min-w-0 gap-2 overflow-visible">
             ...
             <div class="relative flex items-center min-w-0">
               <button data-action="toggle-section-dropdown"
                 class="flex items-center gap-1 min-w-0 rounded px-1 py-0.5 transition-colors ${hasMutipleSections ? 'hover:bg-gray-800 cursor-pointer' : 'cursor-default opacity-70'}"
                 ${!hasMutipleSections ? 'disabled' : ''}
               >
                 <span class="text-sm font-bold text-gray-200 truncate">${activeSection.name}</span>
                 <span class="text-[10px] font-mono text-gray-400 flex-shrink-0">${activeSectionIndex}/${totalSections}</span>
                 ${hasMutipleSections ? ChevronDownIcon(`w-3 h-3 text-gray-400 flex-shrink-0 transition-transform ${state.uiState.sectionDropdownOpen ? 'rotate-180' : ''}`) : ''}
               </button>
               ${state.uiState.sectionDropdownOpen && hasMutipleSections ? `
               <!-- Section Dropdown Panel -->
               <div class="absolute top-full left-1/2 -translate-x-1/2 mt-1.5 min-w-[200px] max-w-[280px] bg-gray-900 border border-gray-700 rounded-lg shadow-2xl shadow-black/50 z-50 overflow-hidden">
                 ${sections.map((s, i) => {
    const isActive = s.id === state.activeSectionId;
    return `
                   <button data-action="select-section-item" data-section-id="${s.id}"
                     class="w-full px-3 py-2.5 flex items-center justify-between gap-2 transition-colors ${isActive
        ? 'bg-cyan-900/30 text-cyan-300 border-l-2 border-cyan-400'
        : 'text-gray-300 hover:bg-gray-800 border-l-2 border-transparent'}"
                   >
                     <span class="text-xs font-medium truncate ${isActive ? 'text-cyan-200' : ''}">${s.name}</span>
                     <span class="text-[10px] font-mono flex-shrink-0 ${isActive ? 'text-cyan-400 font-bold' : 'text-gray-500'}">${i + 1}/${totalSections}</span>
                   </button>`;
  }).join('')}
               </div>
               ` : ''}</div>
