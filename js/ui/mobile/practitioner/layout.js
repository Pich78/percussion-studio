    ...
    const rows = tracks.map((track, tIdx) => {
        ...
        <div class="flex items-center gap-3">
            <button data-action="toggle-mute" data-track-index="${tIdx}"
                class="w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 transition-colors border
            ...
            <span class="w-20 text-xs font-bold truncate flex-shrink-0 ${mix.muted ? 'text-gray-600' : ''}"
                  style="color: ${mix.muted ? '' : (def.color || '#d1d5db')}">${def.name || track.instrument}</span>
            <div class="flex-1 h-6 relative flex items-center group/vol cursor-pointer ${mix.muted ? 'opacity-30' : ''}">
                <div class="absolute left-0 right-0 h-1.5 bg-gray-800 rounded-full border border-gray-700/50"></div>
                <div class="absolute left-0 h-1.5 bg-gradient-to-r from-indigo-500 to-indigo-400 rounded-full pointer-events-none" style="width: ${Math.round(mix.volume * 100)}%"></div>
                <div class="absolute w-4 h-4 bg-white rounded-full shadow-md z-[15] touch-none pointer-events-none" style="left: calc(${Math.round(mix.volume * 100)}% - 8px)"></div>
                <input type="range" min="0" max="1" step="0.01" value="${mix.volume}"
                    data-action="update-volume" data-track-index="${tIdx}"
                    class="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-20" />
            </div>
            <span class="w-9 text-right text-xs font-mono text-gray-500 ${mix.muted ? 'opacity-30' : ''}">${Math.round(mix.volume * 100)}%</span>
        </div>`;
    }).join('');
...
const renderPortraitPlayBar = () => `
    <div class="flex-shrink-0 pb-[env(safe-area-inset-bottom)] bg-gray-950 border-t border-gray-800 px-4 pt-3 pb-4">
