// file: lib/TestRunner.js (Upgraded with beforeEach/afterEach Hooks)
    ...
    expect(actual) {
        const expectObject = {
            ...
            toThrow: (expectedErrorMessage) => {
                ...
                let errorMessage = '';
                try {
                    // This assumes `actual` is a function for `toThrow`
                    actual(); 
                } catch (e) {
                    caught = true;
        ...
        expectObject.not = {
            ...
            // --- FIX: Added the missing not.toThrow implementation ---
            toThrow: () => {
                let caught = false;
                let errorMessage = '';
                try {
                    actual(); // 'actual' is the function passed to expect()
                } catch (e) {
                    caught = true;
                    errorMessage = e.message;
                }
                if (caught) {
                    // If an error was caught, the assertion fails.
                    throw new Error(`Expected function not to throw an error, but it threw: "${errorMessage}"`);
                }
                // If no error was caught, the assertion passes, so we do nothing.
            }
        };
