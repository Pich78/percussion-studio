      ...
      <div class="flex items-center gap-4 flex-shrink-0">
        ...
        <div class="flex items-center gap-3 bg-gray-900 px-3 py-2 rounded-lg border border-gray-800">
          <div class="flex flex-col items-end leading-none">
              ...
              <span class="text-xs font-mono font-bold text-cyan-400" id="header-global-bpm">${Math.round(playback.currentPlayheadBpm)} <span class="text-[9px] text-gray-600">BPM</span></span>
        </div>
          <!-- BPM Slider with Handle (matching volume slider style) -->
          <div class="relative w-40 h-5 flex items-center group/bpm cursor-pointer">
            <!-- Background track -->
            <div class="absolute left-0 right-0 h-2 bg-gray-700 rounded-full cursor-pointer"></div>
            <!-- Fill bar (cyan gradient) -->
            <div class="absolute left-0 h-2 bg-gradient-to-r from-cyan-600 to-cyan-400 rounded-full cursor-pointer" style="width: ${((playback.currentPlayheadBpm - 40) / 200) * 100}%"></div>
            <!-- Handle -->
            <div class="absolute w-4 h-4 bg-white rounded-full shadow-md border border-cyan-400 cursor-pointer z-[15] transition-transform group-hover/bpm:scale-110" style="left: calc(${((playback.currentPlayheadBpm - 40) / 200) * 100}% - 8px)"></div>
